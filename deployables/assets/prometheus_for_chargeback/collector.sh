#!/bin/sh
# Prometheus collector for Chitragupt chargeback metrics.
# Scrapes /metrics in OpenMetrics format, writes TSDB blocks via promtool.
# This script requires the Chitragupt metrics server to serve OpenMetrics format.
...
#
# Required env vars:
#   CHITRAGUPT_METRICS_URL     — URL to /metrics endpoint (e.g. http://localhost:9090/metrics)
#   CHITRAGUPT_HEALTH_URL      — URL to /health endpoint  (e.g. http://localhost:8080/health)
#   TSDB_OUT_DIR               — output directory for TSDB blocks (default: /data/prometheus)
#
# Optional env vars:
#   CHITRAGUPT_METRICS_FORMAT  — "openmetrics" (default) or "text".
...
METRICS_URL="${CHITRAGUPT_METRICS_URL:-http://localhost:9090/metrics}"
HEALTH_URL="${CHITRAGUPT_HEALTH_URL:-http://localhost:8080/health}"
TSDB_OUT_DIR="${TSDB_OUT_DIR:-/data/prometheus}"
METRICS_FORMAT="${CHITRAGUPT_METRICS_FORMAT:-openmetrics}"
...
if [ "${METRICS_FORMAT}" = "openmetrics" ]; then
    ...
    echo "ERROR: CHITRAGUPT_METRICS_FORMAT=text is incompatible with this collector script." >&2
    ...
    echo "ERROR: Unknown CHITRAGUPT_METRICS_FORMAT=${METRICS_FORMAT}. Must be 'openmetrics' or 'text'." >&2
    exit 1
fi

wait_for_ready () {
    echo "Waiting for Chitragupt readiness at ${HEALTH_URL} ..."
    while true; do
...
scrape_interval_seconds () {
    ...
    ts=$(curl -s --max-time 10 -H "${ACCEPT_HEADER}" "${METRICS_URL}" 2>/dev/null \
        | grep -E '^chitragupt_chargeback_amount' \
        | head -1 \
        | awk '{print $NF}' \
        | cut -d'.' -f1)
...
while true; do
    ...
    tmp_metrics=$(mktemp /tmp/chitragupt_metrics_XXXXXX.txt)
