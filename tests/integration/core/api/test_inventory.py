...
class TestInventorySummary:
    ...
    def test_inventory_summary_with_resources(
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["resource_counts"] == {"kafka_cluster": 3}
        assert data["identity_counts"] == {}
    ...
    def test_inventory_summary_with_identities(
        ...
        data = response.json()
        assert data["resource_counts"] == {}
        assert data["identity_counts"] == {"service_account": 2}
    ...
    def test_inventory_summary_combined(self, app_with_backend: TestClient, in_memory_backend: SQLModelBackend) -> None:
        ...
        assert data["resource_counts"]["environment"] == 1
        assert data["resource_counts"]["connector"] == 2
        assert data["resource_counts"]["kafka_cluster"] == 3
        assert data["identity_counts"] == {"service_account": 1}
