...
- Populate mandatory FOCUS `InvoiceIssuerName` from the canonical Confluent participating-entity mapping for eligible Direct-billed PAYG Preview scopes while keeping invoice-assigned IDs unavailable.
- Populate mandatory FOCUS `BillingCurrency` from the normalized operator-configured currency for eligible Direct-billed PAYG Preview scopes. The Confluent Costs API still provides no per-record currency field; the configured value is scope authority rather than per-record inference, non-USD remains fail-closed, and no currency conversion occurs.

### Added
- Add optional self-managed Kafka topic attribution. Topic ingress, egress, and storage use topic-labelled telemetry against broker-wide client cost pools, while the explicit `shared_even_v1` policy can distribute fixed compute evenly across active topics. Exclusions are read-time reporting classification, so configuration reloads reclassify retained results without recalculating their amounts.
- Add bounded, redacted operational diagnostics across API requests, pipeline workflows, providers, metrics, storage, emitters, and FOCUS Mapping Preview. Operators can correlate routine and failure events by request, pipeline run, calculation, revision, repair, date/month, stage, outcome, and retry position; sanitized exception ownership prevents duplicate traceback-bearing events without changing API, financial, persistence, or retry behavior. Operator guidance covers severity, correlation, redaction, volume, and representative recovery actions.
- Add canonical `focus-metadata.json` to every new FOCUS Mapping Preview package with nonstandard `x_ChitraguptaPreview...` generator, dataset-instance, recency, exact-schema, artifact-linkage, and delivery metadata without claiming FOCUS 1.4 conformance. The Chitragupta manifest remains authoritative for checksums, sizes, expiry, revisions, known gaps, and lifecycle. Metadata is validated before Ready/current publication and is served as identical stored bytes across API, CLI, web UI, individual downloads, and archives. Published monthly revisions declare Replacement with Overwrite complete-snapshot semantics and must not be aggregated; requested packages explicitly remain outside a correction series.
- Allow enabled FOCUS Mapping Preview tenants to omit `focus_preview.effective_end_date`. Ad-hoc requests, scheduled publication cycles, and repairs now resolve an exclusive end once from their own UTC creation or cycle date, while an explicit configured end remains a fixed exclusive hard override. Lookback, cutoff, retention, and fail-closed evidence requirements remain independent.
- Expose tenant-scoped FOCUS Mapping Preview readiness as disabled, ready, upgrading, degraded, or unavailable, with durable completed-date/total-date progress and adaptive UI polling. Existing valid Preview data and unrelated application features remain available during repair. Historical repair admission now has finite process-local running and waiting limits, retryable pre-creation capacity rejection, and restart recovery that requires explicit resubmission instead of automatically resuming interrupted work.
- Persist and expose the latest ordinary and Preview evidence retention outcomes independently for each enabled tenant. Readiness now identifies the affected cleanup, attempt time, success or failure, and redacted operator diagnostic; a failure degrades only Preview while existing valid Preview data and unrelated features remain available. A later success clears only the matching retention cause, preserving any independent cleanup or historical repair state.
- Bound FOCUS Mapping Preview generation with process-local capacity shared by requested packages and scheduled tenant-month publication, including finite global/per-tenant queues, an exact retryable capacity 429, safe scheduled deferral, and a configurable 2 GiB per-generation spool ceiling. Existing packages and configuration remain compatible, and manifest/file delivery verifies and streams stored content without loading complete files.
- Make Preview retention and artifact recovery owner-safe and restart-safe. Calculation-owned billing, chargeback, pipeline, source-readiness, and lineage data now retain whole UTC calculation days; lineage cleanup reconciles retained runs against pipeline authority and billing origins idempotently. Requested packages and published revisions use a storage-backend-isolated opaque v1 namespace, one stable lock through metadata commit, authoritative reference rechecks before finalized-orphan deletion, visible retryable recovery failures, and preservation of referenced or unverifiable legacy packages without a database migration.
- Add durable asynchronous REST repair for retained pre-Preview Confluent Cloud calculations. In combined API/worker mode, operators can repair a bounded eligible UTC range from authoritative provider history, track every date through canonical recalculation and Daily/Monthly validation, and retry partial or interrupted operations without fabricating legacy evidence or changing data outside the selected tenant and range.
- Harden FOCUS Mapping Preview deployment boundaries so worker-only mode exposes no Preview HTTP or OpenAPI routes while API and combined modes share the same contract. Cross-client verification confirms that the API, CLI, and web UI retrieve the same stored package bytes and checksums without transformation; deployment guidance now covers shared storage, the external authentication boundary, and the gates required before a future FOCUS Export designation.
- Browse current and superseded published FOCUS revisions by month in the web UI, API, and remote CLI, inspect source freshness and validation evidence, and retrieve a retained revision's manifest, CSV parts, or ZIP directly. Revisions are explicitly complete replacements that must not be aggregated. Published revision cleanup now follows the tenant billing-data retention cutoff, hides eligible revisions before package deletion, and retries failed cleanup across periodic cycles and restarts; requested ad-hoc packages keep their separate fixed seven-day expiry.
- Publish validated Settled Monthly Full FOCUS output after successful periodic pipeline cycles, including settlement-ready header-only months. Active and otherwise incomplete months remain available as on-demand Provisional packages but are excluded before automatic projection. The first Settled result and later material Settled corrections atomically replace the current tenant/month revision while failures preserve the prior package; existing Provisional revisions remain under ordinary supersession and retention. New current-only metadata, manifest, file, and ZIP endpoints use month and revision guards so clients can refetch safely when publication races a download; physical CSV partition changes alone do not create a replacement.
- Complete the requested FOCUS Preview package lifecycle: canonical row ordering, optional deterministic CSV partitioning through `preview.max_csv_file_bytes`, complete `chitragupta.preview-manifest.v1` lifecycle and checksum metadata, recent-request pagination, verified individual downloads, deterministic Download All archives, seven-day expiry, and restart-safe worker leases. The CLI adds no-wait/JSON/status/existing-request download modes with atomic verified local output, while the web UI adds recent history, freshness, expiry, actual part names, and Download All.
- Add Monthly grain and selectable Full, Summary, and Custom profiles to FOCUS Mapping Preview. Monthly requests accept one explicit `YYYY-MM`, freeze provisional/settled evidence coverage at submission, aggregate validated daily rows without rerunning allocation or changing persisted ratios, and record exact effective columns in the manifest. The UI defaults to the current UTC month, the remote CLI supports both grains and all profiles while retaining `daily-full`, and migration 022 adds the persisted current-profile metadata required for fail-closed delivery.
- Add allocation-lineage projection to FOCUS Mapping Preview. Migration 021 links retained raw Cost evidence to the existing `CCloudBillingLineItem` natural key and stores calculation run/portion lineage from actual `CalculatePhase` output without changing allocation policy. The Full profile supports multiple billing origins and actual identity/resource/`UNALLOCATED` portions, exact cost/quantity reconciliation, persisted ratio/method evidence, separate package-time origin/target tags, provider-null promotional allowances, and signed refunds. Missing legacy association requires an ordinary regather plus calculation; invalid lineage and reconciliation failures remain fail-closed with typed diagnostics. TABLEFLOW provider context and multiple native/tier sources under one billing origin remain outside the current scope.
- Add the executable 65-column FOCUS Full profile plus 12 custom evidence columns, persisted Confluent organization billing-account authority, typed service/credit/refund and exact financial projection, deterministic derived SKU evidence, raw provider cloud/region projection, invoice separation, and closed gap validation. Organization-wide unsupported lineage shapes fail closed with `preview_mapping_scope_unsupported`, TABLEFLOW fails `preview_provider_context_incomplete`, and the package remains explicitly non-conforming. Provider organization gathering and per-resource-type deletion safety are isolated without changing global handler or allocator ownership.
- Fail closed on ineligible FOCUS Mapping Preview scopes with optional tenant `focus_preview` Direct-billed PAYG/USD/effective-interval configuration, acquisition-window-aware calculation diagnostics, complete streamed source/aggregate eligibility checks, stable source failure categories, and bounded redacted diagnostic correlations across persistence, API, CLI, and UI. Migration 020 adds nullable correlation persistence. Confluent Costs API records remain without authoritative per-record ISO currency; eligible rows use the normalized configured scope currency for `BillingCurrency`, non-USD is rejected without conversion, and the 364-day `lookback_days` limit remains acquisition-only rather than a retention or reconstruction promise.
- Add asynchronous Daily Full requests for Confluent Cloud, deterministic non-conforming FOCUS 1.4 manifest/CSV packages in atomic durable local storage, canonical status/download APIs, a web UI, and the remote `chitragupta-preview` client. Migration 019 adds per-date calculation identity/provenance and Preview request persistence without backfilling or editing existing collected data.
- Add native Confluent Cost API source-evidence persistence alongside billing aggregates, with provider-backed IDs, deterministic composite/collision identities, tier-level addressability, malformed-row diagnostics, and rolling-window replacement and retention semantics.
...
- Add `GET /api/v1/tenants/{tenant_name}/tags/keys` and `GET /api/v1/tenants/{tenant_name}/tags/keys/{tag_key}/values` endpoints for distinct tag key/value discovery (TASK-217). Supports optional `entity_type` filter on both endpoints and `q` prefix autocomplete on the values endpoint. Returns alphabetically sorted results. Backs the frontend ownership-pivot dropdown and autocomplete UI (TASK-216).
- Add `tag_key` and `tag_value` query params to topic attribution list endpoint (`GET /tenants/{t}/topic-attributions`) for tag-based WHERE filtering (TASK-218). Resource-only subquery matches the chargeback list endpoint pattern. Frontend `TopicAttributionFilterPanel` gains Tag Key and Tag Value input fields; `useTopicAttributionFilters` hook wires them to URL search params.
- Add `GET /api/v1/tenants/{tenant_name}/graph` topology endpoint (TASK-219). Returns cost data in graph shape — nodes (entities with cost, lifecycle timestamps, and tags) and edges (parent-child and charge relationships) — for the cost explorer graph frontend. Three views: root (tenant → environments), environment focus (env + child resources up to configurable depth), and cluster focus (cluster + topics + charged identities with cross-reference metadata). Billing period defaults to the calendar month containing the `at` timestamp; overridable with `start_date`/`end_date`. Raises 404 for unknown focus entities.
- Add graph search, diff, and timeline endpoints (TASK-220). Three new endpoints enabling interactive navigation in the cost explorer graph frontend: `GET /graph/search?q=` (case-insensitive entity search with relevance ordering, max 20 results), `GET /graph/diff` (cost comparison between two date windows with per-node `status=new|deleted|changed|unchanged` and `pct_change`), and `GET /graph/timeline` (daily cost time series with gap filling for sparklines). Entity type routing in timeline: topics use `topic_attribution_facts`, environments group by `env_id`, identities filter by `identity_id`, all other resources group by `resource_id`.
- Add timeline scrubber and cost diff mode to the Cost Explorer graph page (TASK-222). The scrubber renders below the graph with drag-to-scrub interaction, play/pause/replay controls, configurable speed (0.5×–4×) and step size (1 or 3 days), debounced API calls during scrub, and a hover tooltip showing per-entity cost at the hovered date (backed by the `/graph/timeline` endpoint). Play mode at 1-day step prefetches the next 5 frames. Cost diff mode overlays red/green indicators by comparing two configurable date windows (week-over-week, month-over-month, last-30d, or custom via `DatePicker.RangePicker`); deleted resources are synthesized as ghost nodes so they remain visible in the diff view.
- Add full-screen Cost Explorer graph page at `/explorer` route (TASK-221). Force-directed graph visualization using Cytoscape.js renders the cost topology — environments, clusters, topics, and identities as interactive nodes with edges showing parent-child hierarchy, chargeback charge relationships, and topic attribution cost flows. Click-to-drill navigation with animated transitions (node enter/exit/reposition), focus+context fading, breadcrumb trail, cost-proportional node sizing, hover tooltips with entity details, and cross-environment identity indicators via phantom nodes. Graph renderer is abstracted behind `GraphRendererProps` interface for future library swap. Sidebar auto-collapses on mount for maximum graph viewport. New dependencies: `cytoscape`, `cytoscape-cose-bilkent`.
- Add tag overlay, entity search, and shareable URL state to the Cost Explorer graph page (TASK-223). Tag overlay colors nodes by a selected tag key (legend shows top-10 values by cost; click a value to group matching nodes and fade non-matching ones; constrained re-layout pulls same-value nodes together). Entity search (Cmd/Ctrl+K) provides type-ahead search across all resources and identities and navigates on selection. Graph state (focus entity, diff mode and date ranges, scrubber position, active tag key and selected value) is encoded in URL params with push/replace history so browser back/forward works and specific views are bookmarkable; copy-link button copies the current URL to clipboard. Backend: `GET /graph/search` now returns `parent_display_name` on each result for human-readable context in the search dropdown.
- Add grouped cluster view and `expand` query parameter to `GET /tenants/{tenant_name}/graph` (TASK-243). Cluster focus requests with more than 20 child topics or charged identities now return summary group nodes (`topic_group`, `identity_group`) with the top-5 highest-cost entities shown individually — preventing browser-crashing responses for large clusters (previously up to 10MB+). `expand=topics` or `expand=identities` returns all individual nodes for that group (up to 200, with overflow and zero-cost collapsing into summary nodes). Graph nodes gain two optional fields: `child_count` and `child_total_cost` (non-null only on group/summary nodes). The `GET /graph/diff` endpoint is unaffected — it bypasses grouping to ensure accurate cost comparisons across periods.
