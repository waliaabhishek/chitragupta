...
### Added
- Add Monthly grain and selectable Full, Summary, and Custom profiles to FOCUS Mapping Preview (TASK-254.06). Monthly requests accept one explicit `YYYY-MM`, freeze provisional/settled evidence coverage at submission, aggregate validated daily rows without rerunning allocation or changing persisted ratios, and record exact effective columns in the manifest. The UI now defaults to the current UTC month, the remote CLI supports both grains and all profiles while retaining `daily-full`, and migration 022 preserves legacy Daily/Full artifact compatibility.
- Complete TASK-254.05 allocation-lineage projection for FOCUS Mapping Preview: migration 021 links retained raw Cost evidence to the existing `CCloudBillingLineItem` natural key and stores calculation run/portion lineage from actual `CalculatePhase` output without changing allocation policy. Daily Full profile v4 supports multiple billing origins and actual identity/resource/`UNALLOCATED` portions, exact cost/quantity reconciliation, persisted ratio/method evidence, separate package-time origin/target tags, provider-null promotional allowances, and signed refunds. Missing legacy association requires an ordinary regather plus calculation; invalid lineage and reconciliation failures remain fail-closed with typed diagnostics. TABLEFLOW provider context and multiple native/tier sources under one billing origin remain outside the current scope.
