...
- Publish validated Monthly Full FOCUS output after successful periodic pipeline cycles, including eligible header-only months. Provisional logical changes, the first settled result, and later settled corrections atomically replace the current tenant/month revision while failures preserve the prior package. New current-only metadata, manifest, file, and ZIP endpoints use month and revision guards so clients can refetch safely when publication races a download; physical CSV partition changes alone do not create a replacement.
